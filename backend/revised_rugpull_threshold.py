...
try:
    ...
    load_dotenv()   # load ETHERSCAN_API_KEY (and others) from .env at project root
except ImportError:
    pass            # python-dotenv not installed; fall back to shell env / --api-key flag


# ─────────────────────────────────────────────────────────────────────────────
# ADDRESS LISTS
# Update these and re-run the script when the labeled address set changes.
# To override at runtime: --rugpull-addresses <file> --genuine-addresses <file>
# ─────────────────────────────────────────────────────────────────────────────

GENUINE_ADDRESSES: list[str] = [
...
def build_wallet_profile(
    ...
    # Sort by nonce to ensure correct ordering for downstream processing
    ...
    # F2 funding lags are stored per-deployment in DeploymentRecord.funding_lag_hours
    # and aggregated in compute_features(). No accumulator needed here.
        ...
        if dominant_funder:
            ...
            if dominant_txs:
                ...
                if funding_lag_hours >= 0:
                    pass  # stored on DeploymentRecord; compute_features() reads it
...
def main():
    ...
    # Load address lists — use hardcoded defaults if text files are absent
    def _file_or_builtin(filepath: str, builtin: list[str], label: str) -> list[tuple[str, str]]:
        if os.path.exists(filepath):
            loaded = load_addresses(filepath, label)
            if loaded:
                print(f"[INFO] Loaded {len(loaded)} {label} addresses from {filepath}.")
                return loaded
            print(f"[WARN] {filepath} found but empty; using built-in address list.")
        else:
            print(f"[INFO] {filepath} not found; using built-in {label} address list ({len(builtin)} addresses).")
        return [(a.lower(), label) for a in builtin]

    rugpull_addrs = _file_or_builtin(args.rugpull_addresses, RUGPULL_ADDRESSES, "rugpull")
    genuine_addrs = _file_or_builtin(args.genuine_addresses, GENUINE_ADDRESSES, "genuine")
